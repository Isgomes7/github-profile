$(function () {
    function buscarUsuario() {
        const username = $("#user-search").val().trim();
        $("#erro").hide();
        $(".perfil").hide();
        $("#repositorios").html("");
...
<path d="M8 8V13C8 14.8856 8 15.8284 8.58579 16.4142C9.17157 17 10.1144 17 12 17H14" stroke="#97A3B6" stroke-width="2"/>
</svg> ${totalCommits} commits`);
                                    },
                                    error: function () {
                                        $(`#${repoId} .commits`).text("Erro ao buscar commits");
                                    }
                                });
                            });
                        }
                    ...
                    error: function () {
                        $("#repositorios").html("<p>Erro ao carregar repositórios.</p>");
                    }
                });
            },
            error: function () {
    ...
    $("#buscar").on("click", buscarUsuario);
    $("#user-search").on("keypress", function (e) {
        if (e.which === 13) buscarUsuario();
    });
});